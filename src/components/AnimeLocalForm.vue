...
import SearchModal from './AnimeSearchModal.vue'
import SettingsModal from './SettingsModal.vue'

import type { AniyomiEntry } from '../types/aniyomi'
...
function handleSelect (searchEntry: AniyomiEntry) {
  Object.assign(entry, searchEntry)
}

const statusOptions = ref([
  'Unknown', 'Ongoing', 'Completed', 'Licensed', 'Publishing finished', 'Cancelled'
])
...
<template>
  <section class="bg-white shadow-md md:rounded-2xl p-6">
    <form class="flex flex-col gap-2.5" @submit.prevent="handleSubmit" autocomplete="off">
      ...
      <div class="sm:flex gap-4">
        <div class="sm:text-right sm:w-32 shrink-0 mt-2 mb-1 sm:mb-0">
          <label for="studio" class="font-semibold">Animation studios</label>
        </div>
        <div class="flex-1">
          <input
      ...
      <div class="sm:flex gap-4">
        ...
        <div class="flex-1">
          <textarea
            ...
            class="w-full h-32"
            id="description"
            placeholder="Ex. Enter a world in the distant future, where Bounty Hunters roam the solar system…"
          />
        </div>
      </div>
      ...
      <div class="sm:flex gap-4">
        ...
        <div class="flex-1">
          <input
            ...
            id="genre"
            class="w-full"
            placeholder="Ex. Action, Adventure, Drama, Sci-Fi"
          >
          <p class="text-xs text-gray-600 mt-1.5 mb-1">
