...
import * as AL from '../types/animeAnilist'
import { AniyomiEntry, AniyomiStatus } from '../types/aniyomi'
...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'select', entry: AniyomiEntry): void
}>()
...
function studios (title: AL.AnilistMedia): string {
  const studiosText = title.studios.edges
    .map(edge => edge.node.name)
    .sort()
    .join(', ')

  return studiosText.length > 0 ? studiosText : 'Unknown studios'
}
...
function handleSelect () {
  ...
  const STATUS_MAPPING: Record<AL.AnilistStatus, AniyomiStatus> = {
    RELEASING: '1',
    FINISHED: '2',
    NOT_YET_RELEASED: '0',
    CANCELLED: '5'
  ...
  const entry: AniyomiEntry = {
    title: properTitle(selection.value.title),
    author: selection.value.studios.edges
      .map(edge => edge.node.name)
      .sort()
      .join(', '),
    artist: "",
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <div class="title-data">
                  ...
                  <p class="title-studios has-text-grey-dark">{{ studios(title) }}</p>
